...
# Case 2 Description 
No vaccine (BL) / 6-10 / 6-10-14 / 6-10-40
...
# set array 
column.names <- c("NumCases_NoVac","NumCases_Rotarix6_10","NumCases_Rotarix6_10_14","NumCases_Rotarix6_10_40",
                  "NoVacHosp", "Rotarix6_10Hosp","Rotarix6_10_14Hosp","Rotarix6_10_40Hosp",
                  "NoVacDeaths", "Rotarix6_10Deaths","Rotarix6_10_14Deaths","Rotarix6_10_40Deaths",
                  "NoVacCost", "Rotarix6_10Cost","Rotarix6_10_14Cost","Rotarix6_10_40Cost",
                  "NoVacCostInt","Rotarix6_10CostInt","Rotarix6_10_14CostInt","Rotarix6_10_40CostInt",
                  "NoVacCostTrt","Rotarix6_10CostTrt","Rotarix6_10_14CostTrt","Rotarix6_10_40CostTrt",
                  "NoVacDALYs","Rotarix6_10DALYs","Rotarix6_10_14DALYs","Rotarix6_10_40DALYs") 
matrix.names <- c(1:10)
row.names <- c(1:nsim)
yearData <-array(rep(1, nsim*28*10),dim=c(nsim,28,10),
              dimnames = list(row.names,column.names,matrix.names))

params <-array(rep(1, nsim*19),dim=c(nsim,19))
colnames(params)<- c("pr_seek","pr_no_treat","pr_inpat","pr_outpat_MS","pr_outpat_NS",
                     ...
                     "cost_vacc_Rotarix6_10","cost_vacc_Rotarix6_10_14","cost_vacc_Rotarix6_10_40",
                     "delRotarix","cost_in_trt","cost_out_trt_NS","cost_out_trt_MS",
                     "daly_wt_NS","daly_wt_MS","dur_inf") 
params <- as.data.frame(params)

outputArray <-array(rep(1, nsim*46),dim=c(nsim,46))
colnames(outputArray)<- c("NumCases_NoVac","NumCases_Rotarix6_10",
                          "NumCases_Rotarix6_10_14","NumCases_Rotarix6_10_40",
...
### Uncertainty Parameters ###
  ...
  costRotarix = 1.94    # without GAVI 5.65
...
# Calculate Total Cost
  ...
  # + nonsevere cost of outpatient treatment * number outpatient treated / discount rate
  ...
  yearData[i,"NoVacCost",j] =    (yearData[i,"NoVacCostTrt",j] + yearData[i,"NoVacCostInt",j])
...
# Calculate Cost
  # (Cost of 1 dose * number of doses administered + Vaccine Wastage) / discounting rate
  ...
  yearData[i,"Rotarix6_10Cost",j] =   (yearData[i,"Rotarix6_10CostInt",j] + yearData[i,"Rotarix6_10CostTrt",j])
...
# Calculate Cost
  ...
  # cost of inpatient treatment * number of hospitalizations
  # + MS cost of outpatient treatment * n outpatient treated 
  # + NS cost of outpatient treatment * n outpatient treated          
  ...
  yearData[i,"Rotarix6_10_14Cost",j] =    (yearData[i,"Rotarix6_10_14CostInt",j] + yearData[i,"Rotarix6_10_14CostTrt",j])
...
# DALYs for Rotarix6_10_14
  ...
  yearData[i,"Rotarix6_10_14DALYs",j] =  (NSCases_Rotarix6_10_14[j]*dur_inf*daly_wt_NS +
                                        ...
                                        (pr_inpat*pr_death_inpat + pr_outpat_MS*pr_death_outpat +
                                        pr_no_treat*pr_death_notreat)*(Yr4MS6_10_14[i,j]*(29.31892209))) /((1+discount)^(j-1))
...
# calculate CE Ratios - each strategy compared to 6/10 for plotting PSA in CEA Plane     
  # outputArray$CERatioY_NoVac[i] =          (outputArray$NoVacCost[i]-outputArray$NoVacCost[i])
  # outputArray$CERatioX_NoVac[i] =          (outputArray$NoVacDALYs[i]-outputArray$NoVacDALYs[i])
  outputArray$CERatioY_Rotarix6_10[i] =    (outputArray$Rotarix6_10Cost[i]-outputArray$NoVacCost[i])
  outputArray$CERatioX_Rotarix6_10[i] =    (outputArray$NoVacDALYs[i]-outputArray$Rotarix6_10DALYs[i])
  outputArray$CERatioY_Rotarix6_10_14[i] = (outputArray$Rotarix6_10_14Cost[i]-outputArray$NoVacCost[i])
  outputArray$CERatioX_Rotarix6_10_14[i] = (outputArray$NoVacDALYs[i]-outputArray$Rotarix6_10_14DALYs[i])
  outputArray$CERatioY_Rotarix6_10_40[i] = (outputArray$Rotarix6_10_40Cost[i]-outputArray$NoVacCost[i])
  outputArray$CERatioX_Rotarix6_10_40[i] = (outputArray$NoVacDALYs[i]-outputArray$Rotarix6_10_40DALYs[i])
  
  # Insert ICERs Here
  outputArray$ICERatio_Rotarix6_10[i] =    (outputArray$Rotarix6_10Cost[i]-outputArray$NoVacCost[i])/
                                           (outputArray$NoVacDALYs[i]-outputArray$Rotarix6_10DALYs[i])
  outputArray$ICERatio_Rotarix6_10_14[i] = (outputArray$Rotarix6_10_14Cost[i]-outputArray$Rotarix6_10Cost[i])/
                                           (outputArray$Rotarix6_10DALYs[i]-outputArray$Rotarix6_10_14DALYs[i])
...
# Nonsevere Cases
MSCasesAverted_Rotarix6_10    <- NSCases_NoVac-NSCases_Rotarix6_10
MSCasesAverted_Rotarix6_10_14 <- NSCases_NoVac-NSCases_Rotarix6_10_14
MSCasesAverted_Rotarix6_10_40 <- NSCases_NoVac-NSCases_Rotarix6_10_40
# Moderate-Severe Cases
NSCasesAverted_Rotarix6_10    <- MSCases_NoVac-MSCases_Rotarix6_10
NSCasesAverted_Rotarix6_10_14 <- MSCases_NoVac-MSCases_Rotarix6_10_14
NSCasesAverted_Rotarix6_10_40 <- MSCases_NoVac-MSCases_Rotarix6_10_40

# Total Cases
outputArray$CasesAverted_Rotarix6_10    <- outputArray$NumCases_NoVac - outputArray$NumCases_Rotarix6_10
outputArray$CasesAverted_Rotarix6_10_14 <- outputArray$NumCases_NoVac-outputArray$NumCases_Rotarix6_10_14
outputArray$CasesAverted_Rotarix6_10_40 <- outputArray$NumCases_NoVac-outputArray$NumCases_Rotarix6_10_40

outputArray$HospAverted_Rotarix6_10     <- outputArray$NoVacHosp - outputArray$Rotarix6_10Hosp
outputArray$HospAverted_Rotarix6_10_14  <- outputArray$NoVacHosp - outputArray$Rotarix6_10_14Hosp
outputArray$HospAverted_Rotarix6_10_40  <- outputArray$NoVacHosp - outputArray$Rotarix6_10_40Hosp

outputArray$DeathsAverted_Rotarix6_10    <- outputArray$NoVacDeaths - outputArray$Rotarix6_10Deaths
outputArray$DeathsAverted_Rotarix6_10_14 <- outputArray$NoVacDeaths - outputArray$Rotarix6_10_14Deaths
outputArray$DeathsAverted_Rotarix6_10_40 <- outputArray$NoVacDeaths - outputArray$Rotarix6_10_40Deaths

outputArray$DALYsAverted_Rotarix6_10    <- outputArray$NoVacDALYs - outputArray$Rotarix6_10DALYs
outputArray$DALYsAverted_Rotarix6_10_14 <- outputArray$NoVacDALYs - outputArray$Rotarix6_10_14DALYs
outputArray$DALYsAverted_Rotarix6_10_40 <- outputArray$NoVacDALYs - outputArray$Rotarix6_10_40DALYs
...
for (i in 1:nsim) {
  for (j in 1:length(WTP)) {
    ...
    NB_NoVac[i,j]       <- 0
    NB_Rotarix6_10[i,j] <-  ((outputArray$NoVacDALYs[i]-outputArray$Rotarix6_10DALYs[i])*WTP[j])-
                        (outputArray$Rotarix6_10Cost[i]-outputArray$NoVacCost[i])
    NB_Rotarix6_10_14[i,j] <-  ((outputArray$NoVacDALYs[i]-
                        outputArray$Rotarix6_10_14DALYs[i])*WTP[j])-
                        (outputArray$Rotarix6_10_14Cost[i]
                        -outputArray$NoVacCost[i])
    NB_Rotarix6_10_40[i,j] <- ((outputArray$NoVacDALYs[i]-
   ...
   if(NB_NoVac[i,j]>NB_Rotarix6_10[i,j]&NB_NoVac[i,j]>NB_Rotarix6_10_14[i,j]&NB_NoVac[i,j]>NB_Rotarix6_10_40[i,j])
      {P_NB[i,j] <- 1}
    else if(NB_Rotarix6_10[i,j]>NB_NoVac[i,j]&NB_Rotarix6_10[i,j]>NB_Rotarix6_10_14[i,j]&NB_Rotarix6_10[i,j]>NB_Rotarix6_10_40[i,j])
      {P_NB[i,j] <- 2}
     else if(NB_Rotarix6_10_14[i,j]>NB_NoVac[i,j]&NB_Rotarix6_10_14[i,j]>NB_Rotarix6_10[i,j]&NB_Rotarix6_10_14[i,j]>NB_Rotarix6_10_40[i,j])
          {P_NB[i,j] <- 3}
    else if(NB_Rotarix6_10_40[i,j]>NB_NoVac[i,j]&NB_Rotarix6_10_40[i,j]>NB_Rotarix6_10[i,j]&NB_Rotarix6_10_40[i,j]>NB_Rotarix6_10_14[i,j])
      {P_NB[i,j] <- 4}
  }
}
...
for (j in 1:length(WTP)) {
    ...
    if(NB_NoVac_mean[j]>NB_Rotarix6_10_mean[j]&NB_NoVac_mean[j]>NB_Rotarix6_10_14_mean[j]&NB_NoVac_mean[j]>NB_Rotarix6_10_40_mean[j]){
      P_NB_mean[j] <- 1
      CEAF_NoVac[j] <- P_NB_NoVac[j] # If No Vaccination is the preferred strategy, save the P it is preferred
    }
    else if(NB_Rotarix6_10_mean[j]>NB_NoVac_mean[j]&NB_Rotarix6_10_mean[j]>NB_Rotarix6_10_14_mean[j]&NB_Rotarix6_10_mean[j]>NB_Rotarix6_10_40_mean[j]){
      P_NB_mean[j] <- 2
      CEAF_Rotarix6_10[j] <- P_NB_Rotarix6_10[j]# If Rotarix6_10 is the preferred strategy, save the P it is preferred
    }
    else if(NB_Rotarix6_10_14_mean[j]>NB_NoVac_mean[j]&NB_Rotarix6_10_14_mean[j]>NB_Rotarix6_10_mean[j]&NB_Rotarix6_10_14_mean[j]>NB_Rotarix6_10_40_mean[j]){
      P_NB_mean[j] <- 3
      CEAF_Rotarix6_10_14[j] <- P_NB_Rotarix6_10_14[j]# If Rotarix6_10 is the preferred strategy, save the P it is preferred
    }
    else if(NB_Rotarix6_10_40_mean[j]>NB_NoVac_mean[j]&NB_Rotarix6_10_40_mean[j]>NB_Rotarix6_10_mean[j]&NB_Rotarix6_10_40_mean[j]>NB_Rotarix6_10_14_mean[j]){
      P_NB_mean[j] <- 4
      CEAF_Rotarix6_10_40[j] <- P_NB_Rotarix6_10_40[j]# If Rotarix6_10+Rotarix6_10_40aign is the preferred strategy, save the P it is preferred
    }
}
```
...
plot(WTP, P_NB_NoVac, type= "l", col='black', ylab="Probability of being cost-effective",xlab="WTP",
    main="Cost-effectiveness Acceptability Curve\nNo Vaccination v. Rotarix Schedules", ylim=c(0,1))
...
legend("bottomright",legend=c("No vaccination","Rotarix6_10","Rotarix6_10_14","Rotarix6_10_40"),
      col=c("black",'#CA0020',"#006BA4FF",'#ABABABFF'),
...
abline(v=335,col="darkgreen")
text(x=320, y=0.5, srt=90,col="darkgreen", cex=0.75, 'WTP (US $335)')
legend("bottomright",legend=c("No Vaccination","Rotarix6_10 vaccination","Rotarix6_10_14 vaccination","Rotarix6_10_40 vaccination"),
      col=c("black",'#CA0020',"#006BA4FF",'#ABABABFF'),
      lty=1, ncol=1, cex =.7, pt.cex = 1)
dev.off()
```
...
costDiff6_10    <- TotCost_Rotarix6_10 - TotCost_NoVac
costDiff6_10_14 <- TotCost_Rotarix6_10_14 - TotCost_NoVac
costDiff6_10_40 <- TotCost_Rotarix6_10_40 - TotCost_NoVac

Casesav10   <- mean(outputArray$CasesAverted_Rotarix6_10)
Casesav14   <- mean(outputArray$CasesAverted_Rotarix6_10_14)
Casesav40   <- mean(outputArray$CasesAverted_Rotarix6_10_40) 
Hospsav10   <- mean(outputArray$HospAverted_Rotarix6_10)
Hospsav14   <- mean(outputArray$HospAverted_Rotarix6_10_14)
Hospsav40   <- mean(outputArray$HospAverted_Rotarix6_10_40)
DALYsav10   <- mean(outputArray$DALYsAverted_Rotarix6_10)
DALYsav14   <- mean(outputArray$DALYsAverted_Rotarix6_10_14)
DALYsav40   <- mean(outputArray$DALYsAverted_Rotarix6_10_40)
Deathsav10  <- mean(outputArray$DeathsAverted_Rotarix6_10)
Deathsav14  <- mean(outputArray$DeathsAverted_Rotarix6_10_14)
Deathsav40  <- mean(outputArray$DeathsAverted_Rotarix6_10_40)

costCaseAv10        <- costDiff6_10/Casesav10
costCaseAv6_10_14   <- costDiff6_10_14/Casesav14
costCaseAv6_10_40   <- costDiff6_10_40/Casesav40
costHospAv10        <- costDiff6_10/Hospsav10
costHospAv6_10_14   <- costDiff6_10_14/Hospsav14
costHospAv6_10_40   <- costDiff6_10_40/Hospsav40
costDALYAv10        <- costDiff6_10/DALYsav10
costDALYAv6_10_14   <- costDiff6_10_14/DALYsav14
costDALYAv6_10_40   <- costDiff6_10_40/DALYsav40
costDeathAv10       <- costDiff6_10/Deathsav10
costDeathAv6_10_14  <- costDiff6_10_14/Deathsav14
costDeathAv6_10_40  <- costDiff6_10_40/Deathsav40

x <- c(outputArray$CERatioX_Rotarix6_10,outputArray$CERatioX_Rotarix6_10_14,outputArray$CERatioX_Rotarix6_10_40)
y <- c(outputArray$CERatioY_Rotarix6_10,outputArray$CERatioY_Rotarix6_10_14,outputArray$CERatioY_Rotarix6_10_40)
Strategy <- c(rep("Rotarix 6/10",length(outputArray$CERatioX_Rotarix6_10)),
              rep("Rotarix 6/10/14",length(outputArray$CERatioX_Rotarix6_10)),
              rep("Rotarix 6/10/40",length(outputArray$CERatioX_Rotarix6_10)))
data1 <- data.frame(x,y,Strategy)
manual_points <- data.frame(
  x = c(DALYsav10,DALYsav14),  # x-coordinates of the points
  y = c(costDiff6_10,costDiff6_10_14),  # y-coordinates of the points
  label = c(print(round(costDALYAv10,2)),print(round(costDALYAv6_10_14,2))),
  col = c("darkred","royalblue4")  # Colors for the points
)
...
ggplot(data1,aes(x,y,col=Strategy)) + 
  ggtitle("Cost-Effectiveness Plane", subtitle="Governemnt Perspective - Strategies Compared to No Vaccination") + 
  ...
  geom_text(data = manual_points, aes(x = x, y = y, label = label), vjust = 1, hjust = -0.5, size = 5, fontface="bold", color = "black") +
  geom_point(data = manual_points1, aes(x = x, y = y), color = manual_points1$col, size = 3) +
  geom_text(data = manual_points1, aes(x = x, y = y, label = label), vjust = -1, hjust = 1, size = 5, fontface="bold", color = "black") +
  guides(colour = guide_legend(override.aes = list(size = 3))) + 
  scale_color_manual(values = c("#CA0020","#006BA4FF","#ABABABFF"))
...
```{r}
#####################
Number_Averted <- c("No Vaccine", "Rotarix 6/10","Rotarix 6/10/14", "Rotarix 6/10/40")
averted <- c(#cost
             round(TotCost_NoVac,-5),round(TotCost_Rotarix6_10,-5),
             round(TotCost_Rotarix6_10_14,-5),round(TotCost_Rotarix6_10_40,-5),
             #DALYs
             round(DALYs_NoVac,-2),round(DALYs_Rotarix6_10,-2),
             round(DALYs_Rotarix6_10_14,-2),round(DALYs_Rotarix6_10_40,-2),
             #DALYs Averted
             NA,round(DALYsav10,-2),round(DALYsav14,-2),round(DALYsav40,-2),
             #cost per DALY averted
             NA, round(costDALYAv10,2),round(costDALYAv6_10_14,2),round(costDALYAv6_10_40,2),
             #ICER
             "--",round(mean(outputArray$ICERatio_Rotarix6_10),1),
             round(mean(outputArray$ICERatio_Rotarix6_10_14),1),
             round(mean(outputArray$ICERatio_Rotarix6_10_40),1))
             # "--",round(mean(outputArray$ICERatio_Rotarix6_10),1),round(mean(outputArray$ICERatio_Rotarix6_10_14),1),"Strongly Dominated")
z <- as.numeric(averted[1:4])
x <- averted[5:8]
a <- averted[9:12]
b <- as.numeric(averted[13:16])
c <- averted[17:20]
...
table3 |>
  gt() |>
  tab_header(
    title = "Cost-Effectiveness Comparison of Rotarix Vaccine Strategies",
    subtitle = glue::glue("Governemnt Perspective - Compared to No Vaccination ")
  )  |>
