...
# Case 3.2 Description 
What would happen if the program switched to the Rotarix 6/10/14 and was considering the RV3-BB Neonatal Vaccine?
How expensive can the neonatal vaccine be priced per dose and still be cost-effective compared to Rotarix 6/10/14?
...
sens_price <- seq(0,6,by=0.1)
WTPnum = 10

Prob_NB_Sens_Rotarix6_10_14 <- rep(NA,length(sens_price))
...
# set array 
column.names <- c("NumCases_Rotarix6_10_14","NumCases_Neonatal1_6_10",
                  "Rotarix6_10_14Hosp","Neonatal1_6_10Hosp",
                  "Rotarix6_10_14Deaths","Neonatal1_6_10Deaths",
                  "Rotarix6_10_14Cost","Neonatal1_6_10Cost",
                  "Rotarix6_10_14CostInt","Neonatal1_6_10CostInt",
                  "Rotarix6_10_14CostTrt","Neonatal1_6_10CostTrt",
                  "Rotarix6_10_14DALYs","Neonatal1_6_10DALYs") 
matrix.names <- c(1:10)
row.names <- c(1:nsim)
...
colnames(params)<- c("pr_seek","pr_no_treat","pr_inpat","pr_outpat_MS","pr_outpat_NS","pr_no_treat_NS",
                     ...
                     "cost_vacc_Rotarix6_10_14","cost_vacc_Neonatal1_6_10",
...
colnames(outputArray)<- c("NumCases_Rotarix6_10_14","NumCases_Neonatal1_6_10",
                          "Rotarix6_10_14Hosp","Neonatal1_6_10Hosp",
                          "Rotarix6_10_14Deaths","Neonatal1_6_10Deaths",
...
### Uncertainty Parameters ###
  ...
  cost_vacc_Rotarix6_10_14 = 1.94 + delRotarix           #Rotarix6_10_14 vaccine costs per dose with GAVI
  # cost_vacc_Rotarix6_10_14 = 5.65 + delRotarix         #Rotarix6_10_14 vaccine costs per dosewithout GAVI
  cost_vacc_Neonatal1_6_10 = sens_price[p] + delRotarix  #Rotarix6_10_14 vaccine costs per dose
    discount=0.03
  ...
  inc_Rotarix6_10_14 =  MSCases_Rotarix6_10_14[j]
...
outputArray$CasesAverted_Neonatal1_6_10 <- outputArray$NumCases_Rotarix6_10_14 - outputArray$NumCases_Neonatal1_6_10
outputArray$HospAverted_Neonatal1_6_10 <- outputArray$Rotarix6_10_14Hosp - outputArray$Neonatal1_6_10Hosp
outputArray$DeathsAverted_Neonatal1_6_10 <- outputArray$Rotarix6_10_14Deaths - outputArray$Neonatal1_6_10Deaths
...
for (i in 1:nsim) {
  for (j in 1:length(WTP)) {
    ...
    NB_Rotarix6_10_14[i,j] <- 0
    NB_Neonatal1_6_10[i,j] <-  ((outputArray$Rotarix6_10_14DALYs[i]-
    ...
    if(NB_Rotarix6_10_14[i,j]>NB_Neonatal1_6_10[i,j])
      {Prob_NB[i,j] <- 1}
    else if(NB_Neonatal1_6_10[i,j]>NB_Rotarix6_10_14[i,j])
      {Prob_NB[i,j] <- 2}
  }
}

# Initialize vectors for the probability each strategy results in the highest net benefit for each WTP value
Prob_NB_Rotarix6_10_14 <- rep(0,length(WTP))
...
for (j in 1:length(WTP)) {
  Prob_NB_Rotarix6_10_14[j] <- sum(Prob_NB[,j]==1)/nsim
...
for (j in 1:length(WTP)) {
    ...
    if(NB_Rotarix6_10_14_mean[j]>NB_Neonatal1_6_10_mean[j]){
      Prob_NB_mean[j] <- 1
      CEAF_Rotarix6_10_14[j] <- Prob_NB_Rotarix6_10_14[j]       # If Rotarix6_10_14 is the preferred strategy, save the probability it is preferred
    }
    else if(NB_Neonatal1_6_10_mean[j]>NB_Rotarix6_10_14_mean[j]){
      Prob_NB_mean[j] <- 2
      CEAF_Neonatal1_6_10[j] <- Prob_NB_Neonatal1_6_10[j] # If Rotarix6_10_14 is the preferred strategy, save the probability it is preferred
    }
}

sensdf <- data.frame(WTP,Prob_NB_Rotarix6_10_14,Prob_NB_Neonatal1_6_10)

Prob_NB_Sens_Rotarix6_10_14[p] <- sensdf$Prob_NB_Rotarix6_10_14[WTP==WTPnum] 
...
## Sensitivity analysis
```{r}
sensdf1 <- data.frame(sens_price,Prob_NB_Sens_Rotarix6_10_14,Prob_NB_Sens_Neonatal1_6_10)
#at 645 GDP per capita, what is the optimal strategy given different RV3 prices?

ggplot(sensdf1,mapping = aes(x=sens_price,Prob_NB_Sens_Rotarix6_10_14)) + geom_smooth(sensdf1,mapping = aes(x=sens_price,y=Prob_NB_Sens_Rotarix6_10_14),color="#006BA4FF") + geom_smooth(sensdf1,mapping = aes(x=sens_price,y=Prob_NB_Sens_Neonatal1_6_10),color="#FF800EFF") +ggtitle("Sensitivity Analysis of RV3 Price", subtitle="Comapred to Rotarix 6/10/14, Government Perspective") + xlab("Per Dose Price of RV3 ($)") + ylab("P(Preferred Strategy)") + theme(axis.text.x = element_text(face="bold", size=12), axis.text.y = element_text(face="bold", size=12)) + scale_y_continuous(breaks=seq(0,1,0.2))
ggsave("sensitivity14P_10.png", path ="/Users/catherinewenger/Desktop/Rota CEA Manuscript/FiguresFin/Sens14")
...
# plot(sens_price, Prob_NB_Sens_Rotarix6_10_14, type= "l", col='blue', ylab="Probability highest NB",xlab="Price of RV3",
...
# legend("topright",legend=c("Rotarix6_10_14","Neonatal1_6_10"),
#       col=c("blue","maroon"),
#       lty=1, ncol=1, cex =.7, pt.cex = 1)
# dev.off()

max(sensdf1$sens_price[Prob_NB_Sens_Rotarix6_10_14<Prob_NB_Sens_Neonatal1_6_10])
